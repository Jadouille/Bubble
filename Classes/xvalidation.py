...
from sklearn.metrics import accuracy_score,roc_auc_score, f1_score, precision_score, recall_score, precision_recall_curve, roc_curve, average_precision_score;
...
class xvalidation:
	def __init__(self, dataset, probs):
		...
		self.scores={'original':[], 'oversample':[], 'oversample_bubble':[], 'oversample_bubble_original':[], 'smote':[]};
		self.stds={'original':[], 'oversample':[], 'oversample_bubble':[], 'oversample_bubble_original':[], 'smote':[]};
		self.name_datasets=['original', 'oversample','oversample_bubble', 'oversample_bubble_original', 'smote']
		self.av_averagePrecision=[0, 0, 0, 0, 0];
		self.av_precision0=[0, 0, 0, 0, 0];
		self.av_precision1=[0, 0, 0, 0, 0];
		self.av_recall0=[0, 0, 0, 0, 0];
		self.av_recall1=[0, 0, 0, 0, 0];
		self.av_prAucs=[0, 0, 0, 0, 0];
		self.av_f1score0=[0, 0, 0, 0, 0];
		self.av_f1score1=[0, 0, 0, 0, 0];
		self.av_auc=[0, 0, 0, 0, 0];
		self.av_aucProbs0=[0, 0, 0, 0, 0];
		self.av_aucProbs1=[0, 0, 0, 0, 0];
		self.std_averagePrecision=[0, 0, 0, 0, 0];
		self.std_precision0=[0, 0, 0, 0, 0];
		self.std_precision1=[0, 0, 0, 0, 0];
		self.std_recall0=[0, 0, 0, 0, 0];
		self.std_recall1=[0, 0, 0, 0, 0];
		self.std_prAucs=[0, 0, 0, 0, 0];
		self.std_f1score0=[0, 0, 0, 0, 0];
		self.std_f1score1=[0, 0, 0, 0, 0];
		self.std_auc=[0, 0, 0, 0, 0];
		self.std_aucProbs0=[0, 0, 0, 0, 0];
		self.std_aucProbs1=[0, 0, 0, 0, 0];


		self.best_averagePrecision=[0, 0, 0, 0, 0];
		self.best_precision0=[0, 0, 0, 0, 0];
		self.best_precision1=[0, 0, 0, 0, 0];
		self.best_recall0=[0, 0, 0, 0, 0];
		self.best_recall1=[0, 0, 0, 0, 0];
		self.best_prAucs=[0, 0, 0, 0, 0];
		self.best_f1score0=[0, 0, 0, 0, 0];
		self.best_f1score1=[0, 0, 0, 0, 0];
		self.best_auc=[9999999999999999,9999999999999999,9999999999999999,9999999999999999,9999999999999999];
		self.best_aucProbs0=[9999999999999999,9999999999999999,9999999999999999,9999999999999999,9999999999999999];
		self.best_aucProbs1=[9999999999999999,9999999999999999,9999999999999999,9999999999999999,9999999999999999];
	...
	def get_average_scores(self):
		for i in range(0, 5):
			##Compute Accuracies
			self.av_averagePrecision[i]=np.mean(self.averagePrecision[i]);
			...
			self.std_averagePrecision[i]=np.std(self.averagePrecision[i]);
			...
			print('          ',i,'_ : Average Precision-', self.av_averagePrecision[i], ', Precicion 0-', self.av_precision0[i], ', Precision 1-', self.av_precision1[i], ', Recall 1-', self.av_recall1[i], ', Recall 0-', self.av_recall0[i], ', PR Aucs-', self.prAucs[i], ', F1 Score 0-', self.av_f1score0[i], ', F1 Score1-', self.av_f1score1, ', AUC-', self.av_auc[i], ', AUC Probs 0-', self.av_aucProbs0[i], ', AUC Probs 1-', self.av_aucProbs1);

			self.scores[self.name_datasets[i]] = [self.av_averagePrecision[i], self.av_precision0[i], self.av_precision1[i], self.av_recall0[i], self.av_recall1[i], self.av_prAucs[i],self.av_f1score0[i], self.av_f1score1[i], self.av_auc[i], self.av_aucProbs0[i], self.av_aucProbs1[i]]
			self.stds[self.name_datasets[i]] = [self.std_averagePrecision[i], self.std_precision0[i], self.std_precision1[i], self.std_recall0[i], self.std_recall1[i], self.std_prAucs[i],self.std_f1score0[i], self.std_f1score1[i], self.std_auc[i], self.std_aucProbs0[i], self.std_aucProbs1[i]]
	...
	def cross_validate_all(self):
		...
		self.cross_validate_one(self.oversample_tt_set, 1);
		tnow=datetime.datetime.now().replace(microsecond=0);
	...
	def compute_scores(self, ypredict, ypredProbs, yTrue, index):
		best = False;
		averagePrecision = average_precision_score(yTrue, ypredict)
		self.averagePrecision[index].append(averagePrecision);
		...
		if f1score0 > self.best_f1score0[index]:
			best=True
			self.best_averagePrecision[index] = averagePrecision;
		...
		print('          ','Average Precision: ', averagePrecision, ', Precicion 0: ', precision0, ', Precision 1', precision1, ', Recall 1: ', recall1, ', Recall 0: ', recall0, ', PR AUC: ', prAuc,  ', F1 Score 0: ', f1score0, ', F1 Score1: ', f1score1, ', AUC: ', auc, ', AUC Probs 0: ', aucProbs0, ', AUC Probs 1: ', aucProbs1);
